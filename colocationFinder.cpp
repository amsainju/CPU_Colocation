...
void colocationFinder::populateData(std::string datasetFilename) { //pass data.csv as argument
	...
	preprocessingTime += (msecs / 1000.0);
...
void colocationFinder::degree2Processing() {
	...
	Integer numberofColocations = 0;
	for (int i = 1; i <= featureTypes.size()-1; ++i)
	{
		numberofColocations += i;
	}
	Integer degree = 2;
	//h_prevalentInstanceTable = (Integer**)malloc(candiColocCounter*sizeof(Integer*));
	//if (parameter.FilterON_OFF == 1) {
	//	h_prevalentCoarseInstanceTable = (Integer**)malloc(candiColocCounter*sizeof(Integer*));
	//}
	h_prevelentColocationCount = 0;
	//candiColocations = new Integer[degree*numberofColocations];
	candiColocCounter = numberofColocations;
					...
					if (count > 0) {
						...
						for (size_t pos = startIndex; pos <= endIndex; pos++) {
							...
							if (dist <= parameter.thresholdDistance) {
								...
								//intermediate.push_back(instanceID);
...
void colocationFinder::candidateColocationGeneral(Integer degree) {
	Integer lastIndex = degree - 2;
	bool flag;
	//Integer maxColocationEstimate = prevelentColocationCount * (prevelentColocationCount + 1) / 2;
	//candiColocations = new Integer[degree*maxColocationEstimate];
			...
			if (flag && h_prevalantColocations[i*(degree - 1) + lastIndex]< h_prevalantColocations[j*(degree - 1) + lastIndex]) {
				...
				if (!flag) {
					...
					for (Integer l = 0; l < degree; l++) {
						//candiColocations[candiColocCounter*degree + l] = inter[l];
...
void colocationFinder::generateInstanceTableGeneral(Integer degree) {
	...
	for (size_t i = 0; i < candiColocCounter; i++) {
		...
		if (parameter.FilterON_OFF == 1) {
			...
			//coarseIntermediate.shrink_to_fit();
			//coarseCandidateInstanceTables.push_back(coarseIntermediate);
...
//	expResult << datasetFilename << "," << exe << "," << maxInstancesNum << "," << chrono::duration_cast<sec>(totalDuration).count() << "," << parameter.PIthreshold << "," << parameter.thresholdDistance << "," << totalFilteredPatterns << "," << totalCandidatePatterns << "," << (Real)totalFilteredPatterns / totalCandidatePatterns << "," << degree2FilterTime << "," << totalFilterTime << "," << totalRefineTime << "," << totalFilterTime + totalRefineTime << "\n";
	//expResult << datasetFilename << "," << exe << "," << maxInstancesNum << "," << chrono::duration_cast<sec>(difference).count() << "," << parameter.PIthreshold << "," << parameter.thresholdDistance << "," << totalFilteredPatterns << "," << totalCandidatePatterns << "," << (Real)totalFilteredPatterns / totalCandidatePatterns << "," << degree2FilterTime << "," << totalFilterTime << "," << totalRefineTime << "," << totalFilterTime + totalRefineTime + preprocessingTime << "\n";
	expResult << datasetFilename << "," << exe << "," << parameter.PIthreshold <<","<< "1" << "," << preprocessingTime <<"," << totalFilterTime << "," << totalRefineTime << ","<<totalFilterTime+totalRefineTime << "," << processingTime << "," << totalTime <<"\n";
	expResult.close();
}
